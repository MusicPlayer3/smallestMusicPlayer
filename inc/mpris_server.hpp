/*
powered py https://github.com/chrg127/mpris-server
*/
#ifdef __linux__
...
inline std::string playback_status_to_string(PlaybackStatus status)
{
    return playback_status_strings[static_cast<int>(status)];
}
inline std::string loop_status_to_string(LoopStatus status)
{
    return loop_status_strings[static_cast<int>(status)];
}
inline std::string field_to_string(Field entry)
{
...
class Server
{
    ...
    {
        return can_control() && bool(next_fn);
    }
    bool can_go_previous() const
    {
        return can_control() && bool(previous_fn);
    }
    bool can_play() const
    {
        return can_control() && bool(play_fn) && bool(play_pause_fn);
    }
    bool can_pause() const
    {
        return can_control() && bool(pause_fn) && bool(play_pause_fn);
    }
    bool can_seek() const
    {
        return can_control() && bool(seek_fn) && bool(set_position_fn);
    }
...
public:
    ...
    void on_quit(auto &&fn)
    {
        quit_fn = fn;
        prop_changed(MP2, "CanQuit", sdbus::Variant(true));
    }
    void on_raise(auto &&fn)
    {
        raise_fn = fn;
        prop_changed(MP2, "CanQuit", sdbus::Variant(true));
    }
    void on_next(auto &&fn)
    {
        next_fn = fn;
        control_props_changed("CanGoNext");
    }
    void on_previous(auto &&fn)
    {
        previous_fn = fn;
        control_props_changed("CanGoPrevious");
    }
    void on_pause(auto &&fn)
    {
        pause_fn = fn;
        control_props_changed("CanPause");
    }
    void on_play_pause(auto &&fn)
    {
        play_pause_fn = fn;
        control_props_changed("CanPlay", "CanPause");
    }
    void on_stop(auto &&fn)
    {
        stop_fn = fn;
        control_props_changed("CanGoNext", "CanGoPrevious", "CanPause", "CanPlay", "CanSeek");
    }
    void on_play(auto &&fn)
    {
        play_fn = fn;
        control_props_changed("CanPlay");
    }
    void on_seek(auto &&fn)
    {
        seek_fn = fn;
    ...
    void on_fullscreen_changed(auto &&fn)
    {
        fullscreen_changed_fn = fn;
        prop_changed(MP2, "CanSetFullscreen", sdbus::Variant(true));
    }
    void on_loop_status_changed(auto &&fn)
    {
        loop_status_changed_fn = fn;
        control_props_changed("CanGoNext", "CanGoPrevious", "CanPause", "CanPlay", "CanSeek");
    }
    void on_rate_changed(auto &&fn)
    {
        rate_changed_fn = fn;
    }
    void on_shuffle_changed(auto &&fn)
    {
        shuffle_changed_fn = fn;
        control_props_changed("CanGoNext", "CanGoPrevious", "CanPause", "CanPlay", "CanSeek");
    }
    void on_volume_changed(auto &&fn)
    {
        volume_changed_fn = fn;
        control_props_changed("CanGoNext", "CanGoPrevious", "CanPause", "CanPlay", "CanSeek");
    }
    ...
    {
        fullscreen = value;
        prop_changed(MP2, "Fullscreen", sdbus::Variant(fullscreen));
    }
    void set_identity(std::string_view value)
    {
        identity = value;
        prop_changed(MP2, "Identity", sdbus::Variant(identity));
    }
    void set_desktop_entry(std::string_view value)
    {
        desktop_entry = value;
        prop_changed(MP2, "DesktopEntry", sdbus::Variant(desktop_entry));
    }
    void set_supported_uri_schemes(const StringList &value)
    {
        supported_uri_schemes = value;
        prop_changed(MP2, "SupportedUriSchemes", sdbus::Variant(supported_uri_schemes));
    }
    void set_supported_mime_types(const StringList &value)
    {
        supported_mime_types = value;
        prop_changed(MP2, "SupportedMimeTypes", sdbus::Variant(supported_mime_types));
    }
    void set_playback_status(PlaybackStatus value)
    {
        playback_status = value;
        prop_changed(MP2P, "PlaybackStatus", sdbus::Variant(detail::playback_status_to_string(playback_status)));
    }
    void set_loop_status(LoopStatus value)
    {
        loop_status = value;
        prop_changed(MP2P, "LoopStatus", sdbus::Variant(detail::loop_status_to_string(loop_status)));
    }
    void set_shuffle(bool value)
    {
        shuffle = value;
        prop_changed(MP2P, "Shuffle", sdbus::Variant(shuffle));
    }
    void set_volume(double value)
    {
        volume = value;
        prop_changed(MP2P, "Volume", sdbus::Variant(volume));
    }
...
inline Server::Server(std::string_view name) : service_name(PREFIX + std::string(name))
{
...
#define M(f) detail::member_fn(this, &Server::f)
    object->addVTable(sdbus::registerMethod("Raise").implementedAs([&]
                                                                   { if (raise_fn) raise_fn(); }),
                      ...
                      sdbus::registerProperty("CanQuit").withGetter([&]
                                                                    { return bool(quit_fn); }),
                      sdbus::registerProperty("Fullscreen").withGetter([&]
                                                                       { return fullscreen; })
                          .withSetter(M(set_fullscreen_external)),
                      sdbus::registerProperty("CanSetFullscreen").withGetter([&]
                                                                             { return bool(fullscreen_changed_fn); }),
                      sdbus::registerProperty("CanRaise").withGetter([&]
                                                                     { return bool(raise_fn); }),
                      sdbus::registerProperty("HasTrackList").withGetter([&]
                                                                         { return false; }),
                      sdbus::registerProperty("Identity").withGetter([&]
                                                                     { return identity; }),
    ...
    object->addVTable(sdbus::registerMethod("Next").implementedAs([&]
                                                                  { if (can_go_next())             next_fn(); }),
                      sdbus::registerMethod("Previous").implementedAs([&]
                                                                      { if (can_go_previous())         previous_fn(); }),
                      sdbus::registerMethod("Pause").implementedAs([&]
                                                                   { if (can_pause())               pause_fn(); }),
                      sdbus::registerMethod("PlayPause").implementedAs([&]
                                                                       { if (can_play() || can_pause()) play_pause_fn(); }),
                      sdbus::registerMethod("Stop").implementedAs([&]
                                                                  { if (can_control())             stop_fn(); }),
                      sdbus::registerMethod("Play").implementedAs([&]
                                                                  { if (can_play())                play_fn(); }),
                      sdbus::registerMethod("Seek").implementedAs([&](int64_t n)
                                                                  { if (can_seek()) seek_fn(n); })
                          .withInputParamNames("Offset"),
                      sdbus::registerMethod("SetPosition").implementedAs(M(set_position_method)).withInputParamNames("TrackId", "Position"), sdbus::registerMethod("OpenUri").implementedAs(M(open_uri)).withInputParamNames("Uri")

                                                                                                                                                 ,
                      sdbus::registerProperty("PlaybackStatus").withGetter([&]
                                                                           { return detail::playback_status_to_string(playback_status); }),
                      sdbus::registerProperty("LoopStatus").withGetter([&]
                                                                       { return detail::loop_status_to_string(loop_status); })
                          .withSetter(M(set_loop_status_external)),
                      sdbus::registerProperty("Rate").withGetter([&]
                                                                 { return rate; })
                          .withSetter(M(set_rate_external)),
                      sdbus::registerProperty("Shuffle").withGetter([&]
                                                                    { return shuffle; })
                          .withSetter(M(set_shuffle_external)),
                      sdbus::registerProperty("Metadata").withGetter([&]
                                                                     { return metadata; }),
                      sdbus::registerProperty("Volume").withGetter([&]
                                                                   { return volume; })
                      ...
                      sdbus::registerProperty("MaximumRate").withGetter([&]
                                                                        { return maximum_rate; }),
                      sdbus::registerProperty("CanGoNext").withGetter(M(can_go_next)), sdbus::registerProperty("CanGoPrevious").withGetter(M(can_go_previous)), sdbus::registerProperty("CanPlay").withGetter(M(can_play)), sdbus::registerProperty("CanPause").withGetter(M(can_pause)), sdbus::registerProperty("CanSeek").withGetter(M(can_seek)), sdbus::registerProperty("CanControl").withGetter(M(can_control))
...
inline std::unique_ptr<Server> Server::make(std::string_view name)
{
    return std::make_unique<Server>(name);
}
